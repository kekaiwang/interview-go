...
除此之外，`elemsize` 和 `elemtype` 分别表示当前 Channel 能够收发的元素类型和大小；`sendq` 和 `recvq` 存储了当前 Channel 由于缓冲区空间不足而阻塞的 Goroutine 列表，这些等待队列使用双向链表 `runtime.waitq` 表示，链表中所有的元素都是 r`untime.sudog` 结构：
...
在函数的最后会统一更新 `runtime.hchan` 的 `elemsize`、e`lemtype` 和 `dataqsiz` 几个字段。
